...
st.caption(
    ...
    "`LEARNINGS.md`, `ClaudeAdaptationPromptV16.txt`, and script notes so future "
...
st.markdown(
    """
1. **Scenes → Regen with feedback** — appends text into that clip’s `visual_prompt` in `nickandme.json` and logs before/after.
2. **Edit Log → Adaptation V16** — adds a permanent rule under a GUI learnings section (for next full adaptations).
3. **Edit Log → Script notes** — writes `review_feedback/SCRIPT_NOTES.md` so engine changes are intentional, not auto-patched into `generation_script.py`.
