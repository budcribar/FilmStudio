...
def _char_cache_key() -> str:
    ...
    parts = []
    # Prefer active project files from Stage 1 status / engine config
    try:
        ...
        cfg = api.get_config()
        candidates.insert(0, str(cfg.get("blueprint_file") or "blueprint.clips.grok.json"))
    except Exception:
        candidates = [
...
sync: dict = {}
try:
    # Pull Stage 1 seeds into empty Stage 2 blueprint (Buster-style stub)
...
if sync.get("synced"):
    st.caption(
        f"Loaded **{sync.get('count')}** character seeds from Stage 1 into the blueprint."
    )
...
with col_main:
    ...
    if not key:
        st.info(
            "No character seeds yet. Run **Adaptation → Stage 1** first, "
            "or add `character_seed_tokens` to the Stage 2 blueprint."
        )
        st.stop()
                ...
                try:
                    ...
                    if mode in ("book_edit", "book_edit_multi"):
                        ...
                        st.warning(
                            ...
                            "(may not match the book). Prefer book_edit."
                        )
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
        st.caption(
            "Uses `source/book_images/` via Grok **image edit** so likeness matches "
            "the book. **Do not re-run Stage 1** for this — only re-generate variants. "
            "If generation fails, check XAI_API_KEY and book images."
        )
    with b2:
