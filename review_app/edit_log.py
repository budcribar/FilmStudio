...
Records reviewer notes so they can be fed back into:
  - nickandme.json (clip visual_prompt / negative_prompt)
  - ClaudeAdaptationPromptV16.txt (learned rules section)
  - generation_script.py guidance notes (review_feedback/SCRIPT_NOTES.md + optional marker)
"""
...
ADAPTATION_PROMPT = ROOT / "ClaudeAdaptationPromptV16.txt"
...
def add_entry(
    ...
    *,
    ...
    entry: Dict[str, Any] = {
        ...
        "targets": targets
        or ["nickandme.json", "ClaudeAdaptationPromptV16.txt", "generation_script"],
        "applied": {
            "blueprint": False,
...
def append_learnings_md(entry: Dict[str, Any]) -> str:
    ...
    if not LEARNINGS_MD.is_file():
        LEARNINGS_MD.write_text(
            ...
            "`ClaudeAdaptationPromptV16.txt` and prompt rewrites.\n\n",
            encoding="utf-8",
        )
...
def append_script_notes(entry: Dict[str, Any]) -> str:
    ...
    if not SCRIPT_NOTES_MD.is_file():
        SCRIPT_NOTES_MD.write_text(
            "# Pipeline / generation_script notes from review\n\n"
            "Feedback that may require engine changes (continuation, QA, anchors, GUI hooks).\n"
            "Do not auto-edit `generation_script.py` blindly — implement deliberately.\n\n",
            encoding="utf-8",
        )
    block = (
...
def append_adaptation_prompt(entry: Dict[str, Any]) -> str:
    """Append a bullet under the GUI learnings section of ClaudeAdaptationPromptV16.txt."""
