...
/// <summary>
/// Manages the single shared OAuth2 connection FilmStudio uses to upload the WIP movie to
...
/// <c>{workspace}/.filmstudio/youtube_token/</c> (Google.Apis' own FileDataStore format).
/// </summary>
public sealed class YouTubeAuthService
{
    private const string UserId = "filmstudio";
    ...
    public YouTubeAuthService(ProjectStore projects, IOptions<FilmStudioOptions> opts)
    {
        _projects = projects;
    ...
    private GoogleAuthorizationCodeFlow? BuildFlow()
    {
        if (!IsConfigured)
            return null;
        var tokenDir = Path.Combine(_projects.WorkspaceRoot, ".filmstudio", "youtube_token");
    ...
    public string BuildAuthorizationUrl(string state)
    {
        var flow = _flow.Value ?? throw new InvalidOperationException(
            "YouTube OAuth is not configured — set FilmStudio:YouTube:ClientId/ClientSecret/RedirectUri.");
