...
public sealed partial class ProjectStore
{
    ...
    /// <summary>Filesystem-safe project id (owner/name → owner__name).</summary>
    internal static string SanitizeProjectIdForPath(string projectId) =>
        (projectId ?? "").Replace('\\', '_').Replace('/', '__').Replace(':', '_').Trim();

    string GetSceneVersionsRoot() => Path.Combine(_root, "scene-versions");

    string SceneVersionPrefix(string projectId, int sceneNumber) =>
        $"{SanitizeProjectIdForPath(projectId)}__scene{sceneNumber:D3}_";

    /// <summary>Snapshot scene shot-plan for history/revert. Safe for ids containing '/'.</summary>
    public async Task<string?> SnapshotSceneVersionAsync(
        string projectId, int sceneNumber, string? message = null, CancellationToken ct = default)
    {
        var dir = await GetProjectDir(projectId).ConfigureAwait(false);
        ...
        var hash = Convert.ToHexString(System.Security.Cryptography.SHA1.HashData(bytes)).ToLowerInvariant();
        ...
        if (!Directory.Exists(dest))
        {
            ...
            await using (var fs = File.Create(Path.Combine(dest, "meta.json")))
                await System.Text.Json.JsonSerializer.SerializeAsync(fs, meta, cancellationToken: ct).ConfigureAwait(false);
        }
        return hash;
    }

    public async Task<IReadOnlyList<SceneHistoryEntry>> GetSceneHistoryAsync(
        string projectId, int sceneNumber, CancellationToken ct = default)
    {
        var versionsRoot = GetSceneVersionsRoot();
        var list = new List<SceneHistoryEntry>();
        if (!Directory.Exists(versionsRoot)) return list;
        ...
        return list.OrderByDescending(e => e.Utc).ToList();
    }

    /// <summary>Restore scene shot-plan from content-hash snapshot. Null if missing.</summary>
    public async Task<string?> RevertSceneAsync(
